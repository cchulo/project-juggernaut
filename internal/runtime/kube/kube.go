// Package kube is the Kubernetes backend: the gateway expresses "I need a pod for
// (user, server type)" as a Session object plus a per-pod Secret; the controller
// turns that into a Pod. The gateway itself needs no rights on Pods or Secrets
// beyond creating the one Secret, which keeps its RBAC small.
package kube
...
// Backend implements runtime.Backend on top of Session objects.
type Backend struct {
	cl        client.Client
...
// New builds the backend for the sessions namespace.
func New(cfg *rest.Config, cl client.Client, namespace string) (*Backend, error) {
...
func (b *Backend) Ensure(ctx context.Context, spec runtime.SpawnSpec) (*runtime.Status, error) {
	...
	labels := map[string]string{
		...
		jugv1.LabelUserHash:   session.UserHash(spec.Key.Subject),
	...
	sess := &jugv1.Session{
		...
		Spec: jugv1.SessionSpec{
			...
			UserHash:           session.UserHash(spec.Key.Subject),
	...
	return &runtime.Status{Name: name, Phase: session.PhasePending, Started: time.Now()}, nil
}

// Status reads the Session status the controller maintains.
func (b *Backend) Status(ctx context.Context, key session.PodKey) (*runtime.Status, error) {
	var s jugv1.Session
...
func statusOf(s *jugv1.Session) *runtime.Status {
	st := &runtime.Status{Name: s.Name, Phase: session.Phase(s.Status.Phase), Endpoint: s.Status.Endpoint, Message: s.Status.Message}
	if st.Phase == "" {
		st.Phase = session.PhasePending
	}
...
func (b *Backend) List(ctx context.Context) ([]runtime.Status, error) {
	var list jugv1.SessionList
	if err := b.cl.List(ctx, &list, client.InNamespace(b.namespace)); err != nil {
		return nil, err
	}
	out := make([]runtime.Status, 0, len(list.Items))
