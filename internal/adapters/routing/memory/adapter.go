// Package memory is the in-process routing table: the laptop store, and the
// cache tier the redis adapter keeps in front of Redis.
package memory
...
func New(*core.Context) (contracts.RoutingTable, error) { return NewMemory(), nil }
...
// Memory is an in-process RoutingTable.
type Memory struct {
	mu       sync.RWMutex
	pods     map[PodKey]*Pod
...
func (m *Memory) CountPods(_ context.Context, subject, serverType string) (int, int, int, error) {
	...
	for _, p := range m.pods {
		if p.Phase == contracts.PhaseGone || p.Phase == contracts.PhaseTerminating {
			continue
		}
...
func (m *Memory) LastActive(_ context.Context, podName string) (time.Time, error) {
	...
	if !ok {
		return time.Time{}, contracts.ErrNotFound
	}
	return t, nil
}
...
var _ contracts.RoutingTable = (*Memory)(nil)
