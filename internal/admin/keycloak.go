// Package admin serves the operator UI on its own listener (loopback by
// default) and talks to the Keycloak Admin REST API with a dedicated service
// account that holds only the realm-management roles view-users, manage-users,
// query-users, query-groups and view-realm.
package admin
...
// User is the API view of a Keycloak user.
type User struct {
	ID        string    `json:"id"`
...
// KeycloakSession is one IdP session of a user.
...
// Directory is the subset of Keycloak Admin operations the UI needs. It is an
// interface so other IdPs (or tests) can plug in.
type Directory interface {
	...
	Sessions(ctx context.Context, id string) ([]KeycloakSession, error)
...
// HTTPStatus maps gocloak errors to HTTP codes.
